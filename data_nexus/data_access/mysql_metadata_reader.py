...
def mysql_fetch_tables(mysql: MySQLConnection, excluded_tables=[]):
    table_data = []
...
def mysql_database_info(mysql: MySQLConnection):
    try:
        ...
        log_error(f"Error searching metadata for database {mysql.database}: {e}")
        raise e
...
def mysql_metadata_table(mysql: MySQLConnection, table_name):
    try:
        ...
        log_error(f"Error searching metadata for table {mysql.database}.{table_name}: {e}")
        raise e
...
def fetch_table_info(mysql: MySQLConnection, table_name):
    try:
        ...
        try:
            ...
            # log_debug(f"Closing the {mysql.database}.{table_name} table count cursor")
            cursor.close()
    except Exception as e:
        log_error(f"Error searching metadata for table {mysql.database}.{table_name}: {e}")
        raise e
...
def fetch_columns_info(mysql: MySQLConnection, table_name):
    try:
        ...
        log_error(f"Error fetching columns info for table {mysql.database}.{table_name}: {e}")
        raise e
...
def fetch_constraints_info(mysql: MySQLConnection, table_name):
    try:
        ...
        log_error(f"Error fetching constraints info for table {mysql.database}.{table_name}: {e}")
        raise e
...
def fetch_indexes_info(mysql: MySQLConnection, table_name):
    try:
        ...
        log_error(f"Error fetching indexes info for table {mysql.database}.{table_name}: {e}")
        raise e
